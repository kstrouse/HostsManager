...
public class HostsFileService : IHostsFileService
{
    ...
    private readonly string? hostsFilePathOverride;

    public HostsFileService(string? appDataDir = null, string? hostsFilePathOverride = null)
    {
        this.appDataDir = appDataDir ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "HostsManager");
        this.hostsFilePathOverride = hostsFilePathOverride;
    }

    public string GetHostsFilePath()
    {
        if (!string.IsNullOrWhiteSpace(hostsFilePathOverride))
        {
